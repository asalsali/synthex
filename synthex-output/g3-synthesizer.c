...
long process_data(long data_pointer, int count) {
    ...
    for (index = 0; index < count; index++) {
        temp_pointer = *(long *)(data_pointer + (long)index * sizeof(long));
        if (temp_pointer == 0) {
        ...
        long *new_node = (long *)malloc(NODE_SIZE); // Use a new variable for clarity
        if (new_node == NULL) {
        ...
        new_node[0] = temp_pointer; // Store the data pointer
        new_node[1] = size; // Store the size
        new_node[2] = 0; // Next node is null
        *(long *)(current_memory + STRING_OFFSET) = (long)new_node; // Link the new node
        current_memory = (long)new_node; // Move to the new node
